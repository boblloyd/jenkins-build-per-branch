...
class JenkinsJobManagerTests extends GroovyTestCase {
	...
	@Test public void testInitGitApi_noBranchRegex() {
		...
		assert jenkinsJobManager.gitApis
	}
	...
	@Test public void testInitGitApi_gitUrlList_noBranchRegex() {
		JenkinsJobManager jenkinsJobManager = new JenkinsJobManager(gitUrl: "git@dummy.com:company/myproj.git,git@dummy.com:company/myproj2.git", jenkinsUrl: "http://dummy.com")
		assert jenkinsJobManager.gitApis.size() > 1
		assert jenkinsJobManager.gitApis.getAt(0).gitUrl == 'git@dummy.com:company/myproj.git' 
		assert jenkinsJobManager.gitApis.getAt(1).gitUrl == 'git@dummy.com:company/myproj2.git' 
	}

	@Test public void testInitGitApi_gitUrlList_withBranchRegex() {
		JenkinsJobManager jenkinsJobManager = new JenkinsJobManager(gitUrl: "git@dummy.com:company/myproj.git,git@dummy.com:company/myproj2.git", branchNameRegex: 'feature\\/.+|release\\/.+|master', jenkinsUrl: "http://dummy.com")
		assert jenkinsJobManager.gitApis.size() > 1
		assert jenkinsJobManager.gitApis.getAt(0).gitUrl == 'git@dummy.com:company/myproj.git' 
		assert jenkinsJobManager.gitApis.getAt(1).gitUrl == 'git@dummy.com:company/myproj2.git' 
	}
